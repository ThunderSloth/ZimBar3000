...
function medina_get_font(dim)
	...
	for c, p in pairs({titlebar_text = FIXED_TITLE_HEIGHT - 2, room_character = dim.room.y}) do
		local max = 200
		local h, s = 0, 1
		local f = fonts[c]
...
function medina_select_custom_font(font_id, fonts, i)
	if i then
...
function medina_restore_default_font(font_id)
	fdb = sqlite3.open(fonts_database)
...
function medina_restore_every_default_font()
	fdb = sqlite3.open(fonts_database)
...
function medina_update_fonts(msg, id, name, text)
	medina_window_setup(window_width, window_height)
