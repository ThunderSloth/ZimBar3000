...
function smugs_select_custom_font(font_id, fonts, i)
	if i then
...
function smugs_restore_default_font(font_id)
	fdb = sqlite3.open(fonts_database)
...
function smugs_restore_every_default_font()
	fdb = sqlite3.open(fonts_database)
...
function smugs_update_fonts(msg, id, name, text)
	smugs_window_setup(window_width, window_height)
