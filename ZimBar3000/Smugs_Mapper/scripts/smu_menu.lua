...
function smugs_get_title_menu()
    ...
    table.insert(options, function()
        ...
        if restore_default then
			smugs_restore_default_colour(restore_default)
		end
    end)	
	menu = menu..">custom|"
	local section = false
	for k, v in pairsByKeys(smu.colours) do
		...
		if type(v) == "table" then
			...
			for i, vv in ipairs(v) do
				...
				table.insert(options, function()
					smugs_select_custom_colour(vv, k, i)
				end)
			end
			menu = menu.."|"
		else
			table.insert(options, function()
				smugs_select_custom_colour(v, k)
			end)
	...
	table.insert(options, function()
		smugs_restore_every_default_font()
	end)
	table.insert(options, function()
		local font_ids = {}
		...
		if font_id then
			...
			smugs_select_custom_font(font_id, fonts, new_font)
		end
	end)
	menu = menu.."<|<"
