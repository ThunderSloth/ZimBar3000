...
function run_mud_config()    
    ...
    function on_trigger_zoption_removed(name, line, wildcards, styles)
        if #zconfig == 0 then
            ...
            check(AddTrigger("zconfig_added", '^[>]?\\w+ alias "zconfig"', "", trigger_flag.Enabled + trigger_flag.Temporary + trigger_flag.RegularExpression + trigger_flag.OneShot + trigger_flag.KeepEvaluating + trigger_flag.Replace, -1, 0, "", "on_trigger_zconfig_added"))
            Send("alias zconfig "..zconfig.."unalias zconfig")
        end
    end
    
    function on_trigger_zconfig_added(name, line, wildcards, styles)
        check(AddTrigger("zconfig_removed", '^[>]?Successfully unaliased "zconfig":.*$', "", trigger_flag.Enabled + trigger_flag.Temporary + trigger_flag.RegularExpression + trigger_flag.OneShot + trigger_flag.KeepEvaluating + trigger_flag.Replace, -1, 0, "", "on_trigger_zconfig_removed"))
        Send("zconfig")
    end    
    ...
    zoption, zconfig = "", ""
     for k1, t in pairs(options) do
        for k2, v in pairs(t) do
            ...
            check(AddTrigger(name, m, "", trigger_flag.Enabled + trigger_flag.Temporary + trigger_flag.RegularExpression + trigger_flag.OneShot + trigger_flag.KeepEvaluating + trigger_flag.Replace, -1, 0, "", "on_trigger_option"))
        end
        zoption = zoption..k1..";"
    end
    
    check(AddTrigger("playername", "^[>]?Colour playername\\s+= [[](?<p_colour>\\w+)[]].*", "", trigger_flag.Enabled + trigger_flag.Temporary + trigger_flag.RegularExpression + trigger_flag.OneShot + trigger_flag.KeepEvaluating + trigger_flag.Replace, -1, 0, "", "on_trigger_playername"))

    check(AddTrigger("zoption_removed", '^[>]?Successfully unaliased "zoption":.*$', "", trigger_flag.Enabled + trigger_flag.Temporary + trigger_flag.RegularExpression + trigger_flag.OneShot + trigger_flag.KeepEvaluating + trigger_flag.Replace, -1, 0, "", "on_trigger_zoption_removed"))
